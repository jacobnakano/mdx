...
class penhhp(DloGen):   
	...
	def genLayout (self):               
		...
		if abut_l :
			...
			llx =  ptOriginX - dummy_l * ( length + CT_wh + CT_cw * 2 ) - (CT_wh+CT_cw+CT_eg3)
		else :
			llx =  ptOriginX  - (CT_wh+CT_cw+CT_eg3)          

		if abut_r :
			urx = lastGate_urx + max(stretch_r , 0.0 )
		elif dummy_r :
			urx =  lastGate_urx + dummy_r * ( length + CT_wh + CT_cw * 2 ) + (CT_wh+CT_cw+CT_eg3)
		else :
			urx =  lastGate_urx  + (CT_wh+CT_cw+CT_eg3) 

		lly = 0.0;
		ury = width;
		mainDiff = Box(llx,lly,urx,ury)
		
		mainDiffRect = Rect(self.layer.pd,mainDiff)
	

		# Create left PD for Auto Abutment
		if abut_l :
			if stretch_l > 0.0 :
				...
				urx = ptOriginX
				llx = urx - (CT_wh+CT_cw+CT_eg3)
	
		leftDiffBox = Box(llx,lly,urx,ury)
		leftDiffRect = Rect(self.layer.pd, leftDiffBox)
	
		# Create Rt PD for Auto Abutment
		# Create left PD for Auto Abutment
		...
		if abut_r :
			if stretch_r > 0.0 :
				...
				llx = lastGate_urx
				urx =  llx + (CT_wh+CT_cw+CT_eg3)
	
		rightDiffBox = Box(llx,lly,urx,ury)
		rightDiffRect = Rect(self.layer.pd, rightDiffBox)
		
		# Calculate contact enclose using min spacing.
		num_ctx = math.floor( ((width - 2 * CT_eg3 + CT_s) / ( CT_wh + CT_s)) + 1e-18)
		...
		# Create dummy M1 Left
		#baseGateX = ptOriginX - (CT_wh+CT_cw*2) - length
		#lly = ptOriginY + CT_eg3 - M1_eh
		#ury = ptOriginY + width - ( CT_eg3 - M1_eh )
...
#			)
		...
		leftDiffRect.props["abutClass"] = "PENHT33"
		...
		leftDiffRect.props["vxlInstSpacingRule"] = PD_s
		
		rightDiffRect.props["name"] = "right"
		rightDiffRect.props["abutClass"] = "PENHT33"
		...
		rightDiffRect.props["vxlInstSpacingRule"] = PD_s
...
#		
		...
		#  CREATE PI
		llx =     mainDiffRect.getLeft() - PI_ei
		urx =     mainDiffRect.getRight() + PI_ei
		lly =     mainDiffRect.getBottom() - max( PI_el, PO_o2+PI_ep)
		#if fingers > 1 :
		#	ury =  mainDiffRect.getTop()  + max(PI_el,((CT_wh+M1_eh*2)+M1_sv)+PI_ep)
		#else :
		#	ury =  mainDiffRect.getTop()  + max(PI_el, M1_sv+PI_ep)
		ury =  mainDiffRect.getTop()  + max(PI_el,((CT_wh+M1_eh*2)+M1_sv)+PI_ep)
	
		P1_Box  = Box(llx,lly,urx,ury)
		P1_Rect = Rect(self.layer.p1, P1_Box)
	
		##  Create HNVW 
		#llx =     mainDiffRect.getLeft() - PD_em2
		#urx =     mainDiffRect.getRight() + PD_em2
		#ury =     mainDiffRect.getTop() + PD_em2
		#lly =     mainDiffRect.getBottom() - PD_em2
		#HVNW_Box  = Box(llx,lly,urx,ury)
		#HVNW_Rect = Rect(self.layer.hvnw, HVNW_Box)
	        #self.bulkPinId.addShape(HVNW_Rect)
		##  Create HV33
		#llx =     mainDiffRect.getLeft() - HV33_en3
		#urx =     mainDiffRect.getRight() + HV33_en3
		#ury =     mainDiffRect.getTop() + HV33_en3
		#lly =     mainDiffRect.getBottom() - HV33_en3
		#HV33_Box  = Box(llx,lly,urx,ury)
		#HV33_Rect = Rect(self.layer.hv33, HV33_Box)

		#  Create NW 
		llx =     mainDiffRect.getLeft() - PD_em1
		urx =     mainDiffRect.getRight() + PD_em1
		ury =     mainDiffRect.getTop() + PD_em1
		lly =     mainDiffRect.getBottom() - PD_em1
		NW_Box  = Box(llx,lly,urx,ury)
		NW_Rect = Rect(self.layer.nw, NW_Box)
	        self.bulkPinId.addShape(NW_Rect)
		#  Create PCH
		llx =     mainDiffRect.getLeft() - PCH_em
		urx =     mainDiffRect.getRight() + PCH_em
		ury =     mainDiffRect.getTop() + PCH_en
		lly =     mainDiffRect.getBottom() - PCH_en
		PCH_Box  = Box(llx,lly,urx,ury)
		PCH_Rect = Rect(self.layer.pch, PCH_Box)
