...
leMPPDefinition		(GLNW		("GLNW_MPP")		(1)		0) ; *
leMPPDefinition		(GLPW		("GLPW_MPP")		(1)		0) ; *
leMPPDefinition		(GLHVNW		("GLHVNW_MPP")		(1)		0)
leMPPDefinition		(GLHVPW		("GLHVPW_MPP")		(1)		0)
leMPPDefinition		(GLHVDNW	("GLHVDNW_MPP")		(1)		0)
leMPPDefinition		(GRNW		("GRNW_MPP")		(1)		0) ; *
leMPPDefinition		(GRPW		("GRPW_MPP")		(1)		0) ; *
...
;
leMPPRingObject ( GLNW_MPP 	"NDMasterPath"   (M1EncPath NIEncPath NWEncPath)  nil	(CTRectDefault)  nil
;  net name  ExposedParameters
 vss userParams (name netName)
) ; MPP Ring Object

leMPPRingObject ( GLPW_MPP 	"PDMasterPath"   (M1EncPath PIEncPath PWLVSEncPath)  nil	(CTRectDefault)  nil
;  net name  ExposedParameters
 vss userParams (name netName)
) ; MPP Ring Object
...
leMPPRingObject ( GRNW_MPP 	"NDMasterPath"   (M1EncPath NIEncPath NWEncPath)              nil	(CTRectDefault)  nil
;  net name  ExposedParameters
 vss userParams (name netName)
) ; MPP Ring Object

leMPPRingObject ( GRPW_MPP 	"PDMasterPath"   (M1EncPath PIEncPath PWLVSEncPath)              nil	(CT2RectDefault)  nil
;  net name  ExposedParameters
 vss userParams (name netName)
) ; MPP Ring Object
...
;             		name 	            	layer 		purpose 	enclosure 	path-style 	conn    chop
leMPPEnclosedPath 	("PWLVSEncPath"	 	"PWLVS"		drawing 	0.17          	truncate 	t       nil
userParams (name layer purpose enclosure conn chop)
) ; Enclosed Path

;             		name 	            	layer 		purpose 	enclosure 	path-style 	conn    chop
leMPPEnclosedPath 	("NWEncPath"	 	"NW"		drawing 	0.17          	truncate 	t       nil
userParams (name layer purpose enclosure conn chop)
) ; Enclosed Path
...
leMPPSubRect (CTRectDefault  CT drawing 0.12 0.12 t t 0.05 0.05 0.14 min 1
userParams (name layer purpose width height conn chop minEnclosure eol spacing spaceType numRows)
)
leMPPSubRect (CT2RectDefault  CT drawing 0.12 0.12 t t 0.05 0.11 0.14 min 1
userParams (name layer purpose width height conn chop minEnclosure eol spacing spaceType numRows)
)
)
