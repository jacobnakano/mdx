...
class rpolyp(DloGen):   
	...
	def genLayout (self): 
		...
		for i in  range ( 0 ,fingers ) :
			...
			lly =  lly-(CT_wh+PPPO_en3)
			ury =  ury+(CT_wh+PPPO_en3)
		...
		#
		if len(resbod_l) > 0 :
			llx = resbod_l[len(resbod_l)-1].getLeft() - PPPO_en1
		else :
			llx = resBbox[0].getLeft() -PPPO_en1
		#
		if len(resbod_r) > 0 :
			urx = resbod_r[len(resbod_r)-1].getRight() + PPPO_en1
		else :
			urx = resBbox[len(resBbox)-1].getRight() + PPPO_en1
			
		lly = resBbox[0].getBottom() - PPPO_en1
		ury = resBbox[0].getTop() + PPPO_en1
			
		P1Box = Box(llx,lly,urx,ury) 
		P1Rect = Rect(self.layer.p1,P1Box)
		#
		# create PPPO
		#
		llx = llx - PPPO_em3
